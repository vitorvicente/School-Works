"""
    ...
    Uses the Torus, Sphere, and Cylinder Objects.
...
class SceneFour(Component):
    ...
    def initObjects(self):
        """
            ...
            :return: None.
        """
        torus = Component(Point((0, -2, 0)), DisplayableTorus(self.shaderProg, 1, 2, 18, 36, ColorType.SOFTRED))
        torus.setMaterial(MATERIALONE)
    ...
    def initLights(self):
        ...
        lightCube0 = Component(Point((0.0, 3.0, 3.0)), DisplayableCube(self.shaderProg, 0.1, 0.1, 0.1, ColorType.RED))
        lightCube0.renderingRouting = "vertex"

        l1 = Light(Point([0.0, -3.0, 3.0]),
                   np.array((*ColorType.DARKORANGE1, 1.0)), np.array((0, -1, 1)))
        lightCube1 = Component(Point((0.0, -3.0, 3.0)), DisplayableCube(self.shaderProg, 0.1, 0.1, 0.1,
                                                                        ColorType.DARKORANGE1))
        lightCube1.renderingRouting = "vertex"

        l2 = Light(Point([3.0, 3.0, 3.0]),
                   np.array((*ColorType.GREEN, 1.0)), None, np.array((1, 1, 1)), np.array((0.1, 0.1, 0.1)),
                   float(0.01))
        lightCube2 = Component(Point((3.0, 3.0, 3.0)), DisplayableCube(self.shaderProg, 0.1, 0.1, 0.1,
                                                                        ColorType.GREEN))
